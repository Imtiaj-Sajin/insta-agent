...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    ...
    try {
      ...
      try {
        // Insert into `automation` table
        const [automationResult] = await connection.query(
          `INSERT INTO automation (post_id, auto_type, keywords) VALUES (?, ?, ?)`,
          [post_id, auto_type, JSON.stringify(keywords)] // Use JSON.stringify for keywords
        );
        const auto_id = (automationResult as any).insertId;

        // Insert into `RDM` table if `auto_type` includes DM (2 or 3)
        if ((auto_type === 2 || auto_type === 3) && dmAnswers?.length) {
          for (const answer of dmAnswers) {
            await connection.query(
              `INSERT INTO rdm (answer, auto_id) VALUES (?, ?)`,
              [answer, auto_id]
            );
          }
        }
        ...
        if ((auto_type === 1 || auto_type === 3) && commentAnswers?.length) {
          for (const answer of commentAnswers) {
            await connection.query(
              `INSERT INTO rcmnt (answer, auto_id) VALUES (?, ?)`,
              [answer, auto_id]
            );
          }
        }
